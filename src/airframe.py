...
class Airframe:
    ...
    def state_dot(self, time, state):
        # Calculate state_dot for integration
    ...
    # Get moments
    def generateMoments(self, omega, phi, theta, psi, winds):
    ...
    def update(self, omega, winds):
        # update moments and integrate
        self.forces, self.moments = self.generateMoments(omega, self.state[9], self.state[10], self.state[11], winds)
    ...
    def getRotMatrix(self):
        ...
        return np.array([1, sPhi * tTheta, cPhi * tTheta, 0, cPhi, -sPhi, 0, sPhi / cTheta, cPhi / cTheta])

...
def run(x,y, z=45, filename = "", output = "out", ignore = False, weather = None):
    ...
    while f != len(xrefarr):
        ...
        for n in range(len(x_dists)):
            dists[n] = np.sqrt((state[0]-x_dists[n])**2 + (state[1]-y_dists[n])**2)
        ...
        # Get wind from x and y
        # wind = interpolate(state[0], state[1], s_x, s_y, xref, yref, s_weather,e_weather)
...
if __name__ == "__main__":
    ...
    errors = run(coords[:,1], coords[:,0], filename=args._id, output=args.out, weather =  weather)
    print("Done!")
